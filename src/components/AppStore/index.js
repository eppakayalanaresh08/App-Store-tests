...
class AppStore extends Component {
  state = {
    inputSearch: tabsList[0].tabId,
    eachWordEvent: '',
  }

  onClickEventEach = event => {
    this.setState({eachWordEvent: event.target.value})
  }

  eachTabClick = tabId => {
    this.setState({inputSearch: tabId})
  }

  getCategory = eachWordList => {
    const {inputSearch} = this.state
    const appListItems = eachWordList.filter(
      eachappList => eachappList.category === inputSearch,
    )
    return appListItems
  }

  getCountrySearch = () => {
    const {eachWordEvent} = this.state
    const countrySearch = appsList.filter(eachItemWord =>
      eachItemWord.appName.toLowerCase().includes(eachWordEvent.toLowerCase()),
    )
    return countrySearch
  }

  render() {
    const {inputSearch, eachWordEvent} = this.state
    const eachWordList = this.getCountrySearch()
    const appListItems = this.getCategory(eachWordList)
        ...
        <div className="container-app">
          ...
          <div className="search-container">
            <input
              ...
              onChange={this.onClickEventEach}
            />
            <img
          ...
          <ul className="tab-list-container">
            {tabsList.map(eachTab => (
              <TabItem
                TabeachItem={eachTab}
                eachTabClick={this.eachTabClick}
                key={eachTab.tabId}
                isActive={eachTab.tabId === inputSearch}
              />
            ))}
          </ul>

          <ul className="App-list-container">
            {appListItems.map(eachApp => (
              <AppItem AppeachItem={eachApp} key={eachApp.appId} />
            ))}
          </ul>
        </div>
      </div>
